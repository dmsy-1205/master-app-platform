...
## STEP12-v2 운영 관리 규칙
관리자는 앱 등록 후에도 앱 정보를 수정할 수 있어야 합니다. 공지 알림 요청 게시판은 운영자가 삭제하거나 상태를 변경할 수 있어야 하며 사용자 화면에는 최신 상태만 보이도록 유지합니다. 모든 삭제 기능은 확인창을 거쳐 실행합니다.


## STEP13 운영 규칙
MasterOS는 이제 서비스 플랫폼 구조로 확장한다.
관리자는 앱 상세 공식 앱 배포 상태 헬스 상태 백업 개발 검사를 한 곳에서 관리한다.
기존 기능을 깨뜨리지 않고 관리자 패널 단위로 확장한다.

---

# STEP14 개발 규칙 추가

STEP14부터 Master App Platform은 MasterOS Platform으로 확장한다.

개발 방향:

- 단순 App Launcher가 아니라 Web OS 운영 플랫폼으로 확장
- 기존 기능은 보존하고 신규 센터는 모듈 단위로 추가
- 관리자 전용 기능은 Firebase admins 및 users.role 기준으로 보호
- 신규 STEP14 기능은 `step14.js`처럼 독립 모듈로 추가하여 기존 admin.js 위험 변경을 최소화
- App Detail, Official, Deployment, Health, Backup, Developer, Statistics, Security 기능은 Firebase 실시간 데이터와 연결
